<template>
  <div id="deals" class="rectangle-1a" data-design-element="desktop-deals-panel">
    <div class="frame-1b">
      <div class="rectangle-1c"></div>
      <div class="rectangle-1d"></div>
      <div class="rectangle-1e"></div>
      <div class="rectangle-1f"></div>
    </div>
    <span class="explore-save">EXPLORE &amp; SAVE</span>
    <div class="frame-20">
      <div class="group-21">
        <div class="group-22">
          <div class="handpicked-deals">
            <span class="handpicked">Handpicked </span>
            <span class="deals-23">deals</span>
            <span class="handpicked-24">.</span>
          </div>
          <div class="vector-25"></div>
        </div>
        <span class="local-prices">Local prices.</span>
      </div>
      <span class="get-up-to">
        Get up to 20% off on amazing stays, tours, <br />restaurants, and more. Only with <br />local businesses.
      </span>
    </div>
    <div class="group-26"></div>
    <div class="group-27"></div>
    <div class="rectangle-28"><span class="minus-20">-20%</span></div>
    <div class="rectangle-29">
      <div class="frame-2a">
        <span class="boutique-hotel">Boutique Hotel</span>
        <div class="from-night">
          <span class="from">from </span><span class="euro-from">&#8364;89</span><span class="per-night"> /night</span>
        </div>
      </div>
      <div class="frame-2b">
        <div class="star-2c"></div>
        <span class="rating">4.9 (128)</span>
      </div>
    </div>
    <div class="frame-2d">
      <div class="frame-2e">
        <div class="price-list"></div>
        <div class="frame-2f">
          <span class="better-prices">Better prices</span>
          <span class="every-time">Every time</span>
        </div>
      </div>
      <div class="frame-30">
        <div class="filler"></div>
        <div class="frame-31">
          <span class="hidden-gems">Hidden gems</span>
          <span class="locals-know">Only locals know</span>
        </div>
      </div>
      <div class="frame-32">
        <div class="present"></div>
        <div class="frame-33">
          <span class="unforgettable">Unforgettable</span>
          <span class="experiences">Experiences</span>
        </div>
      </div>
    </div>
    <div class="group-34"></div>
    <a class="frame-35" href="#book">
      <span class="how-it-works">See how it works</span>
      <div class="arrow-36"></div>
    </a>
  </div>
</template>
